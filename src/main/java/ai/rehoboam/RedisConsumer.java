...
public class RedisConsumer {
    ...
    public static void main(String[] args) throws Exception {
        ...
        new Random().nextBytes(someInputBytes);
        ...
        Map.Entry<String, StreamEntryID> streamQeury = new AbstractMap.SimpleImmutableEntry<>("conflation", new StreamEntryID());

        List<Map.Entry<String, List<StreamEntry>>> range= jedis.xread(1, 1L, streamQeury);
        for(Map.Entry<String,List<StreamEntry>> entry: range){
            System.out.println("topic = " + entry.getKey());

            for(int i=0;i<entry.getValue().size();i++){
                Map<String, String> fields = entry.getValue().get(i).getFields();
                Map.Entry<String, String> stringEntry = fields.entrySet().iterator().next();

                System.out.println("key   = " + stringEntry.getKey());
                for(int j=0;j<stringEntry.getValue().getBytes().length;j++){
                    System.out.print(stringEntry.getValue().getBytes()[j]);
                }System.out.println();
            }
        }
        jedis.close();
    ...
    public void process(String key, ConflationEventWrapper cew) {
        byte[] value = new byte[10];
//        byte[] value = linkageStore.get("conflation", Bytes.wrap(key.getBytes(Charset.forName(Constants.UTF8))));
    ...
    private ConflationMetadataWrapper getConflationMetadataWrapper(String key, byte[] value) {
        if (value == null) {
        ...
        return new ConflationMetadataWrapper(value);
    }

    private void sendSendDataEvent(String key, ConflationEventWrapper cew, ConflationMetadataWrapper cmw) {
...
//        }
    ...
    private void initiateConflationSequence(String key, ConflationMetadataWrapper cmw, ConflationEventWrapper cew) {
    ...
    private void sendChangeRoot(String key, ConflationEventWrapper cew) {
    ...
    private void handleSendDataEvent(String key, ConflationEventWrapper cew, ConflationMetadataWrapper cmw) {
    ...
    private void handleChangeRootEvent(String key, ConflationMetadataWrapper cmw, ConflationEventWrapper cew) {
    ...
    private void deleteDataForTgid(String tgid) {
...
//        }
    ...
    private void restoreStores(List<StoreDataProto.StoreData> storeDataList, String tgid) {
...
//        }
    ...
    private Map<String, List<StoreDataProto.StoreData>> buildStoreDataMap(String tgid, ConflationMetadataWrapper cmw) {
...
//    }
    ...
    private void putConflationMetadata(String key, ConflationMetadataWrapper cmw) {
//        byte[] conflationMetadataBytes = cmw.serializeConflationMetadataWrapper();
//        linkageStore.put("conflation", Bytes.wrap(key.getBytes(Charset.forName(Constants.UTF8))), conflationMetadataBytes);
    }

    private void postConflation(String tgid, HashSet<String> newTids) {
